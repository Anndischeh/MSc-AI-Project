...
# NEW: imports for anomaly evaluation & saving sample outputs
...
def train_autoencoder(model, train_loader, optimizer, criterion, device, epochs, model_name="Autoencoder"):
    model.train()
...
def evaluate_autoencoder(model, test_loader, device):
    """
    Returns:
        reconstruction_errors: np.array, shape (N,)
    ...
    labels = []
    with torch.no_grad():
        for data, label in tqdm(test_loader, desc="Evaluating AE", leave=False):
            data = data.to(device)
            output = model(data)
...
def plot_reconstruction_error(errors, labels, results_dir, model_name="autoencoder"):
...
# NEW: generic helper to compute & save ROC, AUC and sample outputs
def compute_and_save_metrics(scores, labels, model_name, results_dir, num_sample_outputs=100):
    ...
    # Sample outputs (for thesis visualization / tables)
...
# --- Datasets & loaders ---
# NEW: for anomaly detection, train the AE only on normal slices
train_dataset = BraTSDataset(
    root_dir=DATA_PATH,
    num_patients=N_PATIENTS_TO_USE,
    img_size=IMG_SIZE,
    train=True,
    normal_only=True  # <-- key for unsupervised anomaly detection
)
...
test_loader_ae = DataLoader(test_dataset, batch_size=BATCH_SIZE, shuffle=False)

# --- Model results container (for multi-model comparison) ---
model_results = []  # list of dicts: {"model": name, "roc_auc": value}

# ==============================================================================
# AUTOENCODER AS MODEL 1
# ==============================================================================

model_name_ae = "Autoencoder"

# Initialize model, optimizer, and loss function
model_ae = Autoencoder(img_size=IMG_SIZE, z_dim=Z_DIM).to(DEVICE)
optimizer_ae = optim.Adam(model_ae.parameters(), lr=LEARNING_RATE)
criterion_ae = nn.MSELoss()

# Train model
train_autoencoder(model_ae, train_loader, optimizer_ae, criterion_ae, DEVICE, EPOCHS, model_name=model_name_ae)

# Evaluate model -> reconstruction errors = anomaly scores
errors_ae, labels_ae = evaluate_autoencoder(model_ae, test_loader_ae, DEVICE)

# Plot reconstruction error distributions
plot_reconstruction_error(errors_ae, labels_ae, RESULTS_DIR, model_name=model_name_ae.lower())

# Compute ROC, AUC, and save sample outputs
roc_auc_ae = compute_and_save_metrics(
    scores=errors_ae,
    labels=labels_ae,
    model_name=model_name_ae,
    results_dir=RESULTS_DIR,
    num_sample_outputs=100  # change if you want more/less
)

# Save trained model weights with model name
torch.save(model_ae.state_dict(), os.path.join(RESULTS_DIR, f"{model_name_ae}_model.pth"))

# Register result for global comparison
model_results.append({
    "model": model_name_ae,
    "roc_auc": float(roc_auc_ae)
})

# ==============================================================================
# 5. GLOBAL COMPARISON TABLE (extend this when you add more models)
# ==============================================================================

df_results = pd.DataFrame(model_results)
df_results.to_csv(os.path.join(RESULTS_DIR, "model_comparison_roc_auc.csv"), index=False)

print("\n=== Model Comparison (ROC-AUC) ===")
print(df_results)
print(f"\nSaved model comparison table to: {os.path.join(RESULTS_DIR, 'model_comparison_roc_auc.csv')}")
